...
using System.Text;
using ASPNETCoreTemplate.Core.Abstractions.SeedWork;
using ASPNETCoreTemplate.Core.Attributes;
using ASPNETCoreTemplate.Core.Configurations;
using ASPNETCoreTemplate.Core.Constants;
using ASPNETCoreTemplate.IoC.Abstractions.SeedWork;
using ASPNETCoreTemplate.IoC.Constraints;
...
namespace ASPNETCoreTemplate.IoC.Extensions;
...
public static class ServiceCollectionExtensions {
  private static readonly Assembly _applicationAssembly = AppDomain.CurrentDomain
    .GetAssemblies().FirstOrDefault(assembly => assembly.GetName().Name == "ASPNETCoreTemplateApplication") ?? Assembly.GetExecutingAssembly();

  private static readonly Assembly _coreAssembly = AppDomain.CurrentDomain
    .GetAssemblies().FirstOrDefault(assembly => assembly.GetName().Name == "ASPNETCoreTemplate.Core") ?? Assembly.GetExecutingAssembly();

  private static readonly Assembly _iocAssembly = AppDomain.CurrentDomain
    .GetAssemblies().FirstOrDefault(assembly => assembly.GetName().Name == "ASPNETCoreTemplate.IoC") ?? Assembly.GetExecutingAssembly();
